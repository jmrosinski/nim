...
SUBROUTINE solveiThLS(nob,nbf,b,amtx,ipn,ips)
!**************************************************************************
!
...
!MWG: workaround due to F2C bug
  ...
  real(rt), INTENT(IN)   :: amtx(1:NZ,nob,nob+1)
  real(rt)               :: wk
  INTEGER                :: k
...
#include <gptl.inc>
  ...
  ret = gptlstart_handle ('solveiThLS', handle)
#endif
...
!$acc loop vector
  ...
  enddo !k-loop
#ifdef FINEGRAINED_TIMING
  ret = gptlstop_handle ('solveiThLS', handle)
#endif
